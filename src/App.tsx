    ...
    const checkL = setInterval(() => {
      if (window.L && mapRef.current) {
        ...
        if (!mapInstance.current) {
          ...
          window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap contributors' }).addTo(mapInstance.current);
        }
        mapInstance.current.eachLayer((layer: any) => { if (layer instanceof window.L.Marker) mapInstance.current.removeLayer(layer); });
        if (matches && matches.length > 0) {
          const bounds = window.L.latLngBounds();
          matches.forEach((m: any) => {
            const date = new Date(m.time);
            window.L.marker([m.lat, m.lon]).addTo(mapInstance.current).bindPopup(`<div style="font-family: ui-sans-serif, system-ui, sans-serif; padding: 4px;"><div style="color: #e11d48; font-weight: 700; font-size: 14px; margin-bottom: 4px;">📍 Point</div><div style="color: #4b5563; font-size: 12px;">${date.toLocaleDateString()} ${date.toLocaleTimeString()}</div><div style="color: #111827; font-size: 13px; font-weight: 500; margin-top: 4px;">Dist: ${m.distance}m</div></div>`);
...
const GeminiStory = ({ match, lang, t }: { match: any, lang: string, t: any }) => {
  ...
  const generateShortStory = async () => {
    ...
    let promptParams = "";
    if (lang === 'pl') {
      promptParams = "Napisz maksymalnie 2 krótkie zdania podsumowania po polsku, gdzie się minęli na podstawie tych współrzędnych.";
    } else {
      promptParams = "Write max 2 short sentences in English summarizing where they crossed paths based on these coordinates.";
    }
    ...
    try {
      ...
      setShortStory(lang === 'pl' ? "Magia AI chwilowo zawiodła (brak klucza)." : "AI magic failed (missing API key).");
    } finally {
  ...
  const generateLongStory = async () => {
    ...
    let promptParams = "";
    if (lang === 'pl') {
      promptParams = "Napisz 4-5 zdań intrygującej historii po polsku. Styl: ciepły, poetycki.";
    } else {
      promptParams = "Write 4-5 sentences of an intriguing, romantic story in English about how they might have passed each other here.";
    }
    ...
    try {
      ...
      // Ignorujemy błędy i po prostu zostawiamy stare dane jeżeli błąd
    } finally {
  ...
  const handleShare = async () => {
    ...
    try {
      // Magia: Zmieniamy klasę na czas robienia screenshota, aby ukryć przyciski i powiększyć marginesy!
      element.classList.add('exporting-card');
      const canvas = await window.html2canvas(element, { 
        backgroundColor: null, 
        scale: 3, // Wysoka rozdzielczość na Retinę / Instagram
        useCORS: true
      });
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 p-5 rounded-2xl border border-indigo-100/50 dark:border-indigo-800/30 shadow-sm relative overflow-hidden">
          ...
          {longStory && (
            <div className="mt-5 pt-5 border-t border-indigo-200 dark:border-indigo-800/50 animate-fade-in relative z-10">
              ...
              {/* NOWY PRZYCISK EXPORTU NA INSTAGRAM */}
...
export default function App() {
  ...
  // ZARZĄDZANIE MOTYWEM KIEROWANYM KLASĄ (Nadpisuje systemowe media queries)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // ŁADOWANIE BIBLIOTEKI DO SCREENSHOTÓW (html2canvas)
  ...
  const initPeerJS = async (): Promise<string> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const peer = new window.Peer(idToUse, { config: { iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] } });
  ...
  const handleFileUpload = (e: any, setFileFn: any, setErrorFn: any) => {
    setErrorFn(null);
    const file = e.target.files[0];
    if (file) {
      if (file.name.toLowerCase().includes('settings.json')) { setErrorFn("To plik Settings. Szukaj Records.json."); e.target.value = ''; return; }
  ...
  const generateFakePartner = (sourceData: any[], setTargetFn: any) => {
    if (!sourceData || sourceData.length === 0) return;
    const basePt = sourceData[Math.floor(sourceData.length * Math.random())];
    let fakePath = []; let curLat = basePt.lat + 0.005; let curLon = basePt.lon + 0.005;
    for (let i = -60; i <= 60; i++) {
            ...
            <div className="w-full max-w-5xl bg-white dark:bg-gray-800 rounded-[2rem] p-8 sm:p-10 border border-gray-200 dark:border-gray-700 mb-12 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
                  ...
                  <input type="range" min="5" max="5000" step="5" value={maxDistance} onChange={(e: any) => setMaxDistance(parseInt(e.target.value))} className="w-full h-2 bg-gray-300 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                </div>
                <div className="bg-gray-50 dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
                  <label className="flex justify-between text-sm font-bold text-gray-700 dark:text-gray-300 mb-6">
                  ...
                  <input type="range" min="1" max="1440" step="1" value={maxTimeDiff} onChange={(e: any) => setMaxTimeDiff(parseInt(e.target.value))} className="w-full h-2 bg-gray-300 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-rose-500" />
                </div>
              </div>
